...
const findAllElements = (context) => {
  ...
  return doSearch(context.dynamodb, context.tableName, scanParams).then(
    (items) => {
      context.items = items;
      return context;
    },
 ...
 */
export const purgeTable = (tableName, dynamodb) => {
  const context = {
    tableName,
    dynamodb,
  };

  return findPrimaryKeys(context).then(findAllElements).then(deleteAllElements);
};
